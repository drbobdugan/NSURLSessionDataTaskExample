...
- (IBAction)buttonPressed:(id)sender
{
    ...
    self.time.text = @"0";
    self.completePercentage.text = @"0";
    ...
    //
    // Configure data task
    // NOTE: You need an HTTP server to respond to the data task request.
    // My server is a very basic python script that is waiting for a POST.
    //
...
totalBytesExpectedToSend:(int64_t)totalBytesExpectedToSend
{
    ...
    // Update UI
    dispatch_block_t work_to_do = ^{
        self.completePercentage.text = [NSString stringWithFormat:@"%.3f", progress*100];
